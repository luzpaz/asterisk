...
static char *handle_showmaneventq(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"	Prints a listing of all events pending in the Asterisk manager\n"
			"event queue.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
...
static struct test_filter_data parsing_filter_tests[] = {
	...
	{ "eventfilter(name(yyy),header(VarSet),method(contains)", "XXX", FILTER_FORMAT_ERROR, { 0, }, TEST_EVENT_NONE, 0},
